...
# hyper-parameters follows OpenCV convention


def distributionAugmentation(window, kde):
    """
    This function is to augment the image window selected by the SIFT by probability density distribution
    :param window: the image window selected by SIFT
    :return: the augmement
    """
    shape_ = window.shape
    flatten_window = window.flatten()
    for idx, pixel in enumerate(flatten_window):
        flatten_window[idx] = kde.my_kde(pixel)
    return flatten_window.reshape(shape_)


def getKeypointsAndDescriptorsBySIFT(image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
    """
    Find SIFT keypoints and descriptors for the input image:image
    """
    image = image.astype('float32')
    base_image = imagePreprocess(image, sigma, assumed_blur)
    num_octaves = getNumberOfOctaves(base_image.shape)
...
def imagePreprocess(image, sigma, assumed_blur):
    """
    Pre-process and normalise image from input image by upsampling by 2 in both directions and blurring
    """
...
def getNumberOfOctaves(image_shape):
    """
    Calculate the number of octaves in image pyramid as function of base image shape (OpenCV default)
    """
...
def findScaleSpaceExtrema(gaussian_images, dog_images, num_intervals, sigma, image_border_width,
    ...
    """
    threshold = floor(0.5 * contrast_threshold / num_intervals * 255)  # normalize using the approach in OpenCV
    keypoints = []
...
def computeGradientAtCenterPixel(pixel_array):
    ...
    # given h = 1, so the formula simplifies to f'(x) = (f(x + 1) - f(x - 1)) / 2
...
def computeKeypointsWithOrientations(keypoint, octave_index, gaussian_image, radius_factor=3, num_bins=36,
    ...
    tensor_gaussian_image = torch.from_numpy(gaussian_image).to(device)
        ...
        if region_y > 0 and region_y < image_shape[0] - 1:
            for j in range(-radius, radius + 1):
                ...
                if region_x > 0 and region_x < image_shape[1] - 1:
                    dx = tensor_gaussian_image[region_y, region_x + 1] - tensor_gaussian_image[region_y, region_x - 1]
                    dy = tensor_gaussian_image[region_y - 1, region_x] - tensor_gaussian_image[region_y + 1, region_x]
                    gradient_magnitude = torch.sqrt(dx * dx + dy * dy)
                    gradient_orientation = torch.rad2deg(torch.arctan2(dy, dx))
                    weight = exp(weight_factor * (
                                i ** 2 + j ** 2))  # constant in front of exponential can be dropped because we will find peaks later
                    histogram_index = int(round(gradient_orientation.item() * num_bins / 360.))
                    raw_histogram[histogram_index % num_bins] += weight * gradient_magnitude.item()
...
def generateDescriptors(keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3,
    ...
    for keypoint in keypoints:
        ...
        gaussian_image = torch.tensor(gaussian_images[octave + 1, layer]).to(device)
        ...
        histogram_tensor = torch.zeros((window_width + 2, window_width + 2, num_bins)).to(
            device)  # first two dimensions are increased by 2 to account for border effects
            ...
            for col in range(-half_width, half_width + 1):
                ...
                if row_bin > -1 and row_bin < window_width and col_bin > -1 and col_bin < window_width:
                    ...
                    if window_row > 0 and window_row < num_rows - 1 and window_col > 0 and window_col < num_cols - 1:
                        ...
                        gradient_magnitude = torch.sqrt(dx * dx + dy * dy)
                        gradient_orientation = torch.rad2deg(torch.arctan2(dy, dx)) % 360
                        weight = exp(weight_multiplier * ((row_rot / hist_width) ** 2 + (col_rot / hist_width) ** 2))
                        row_bin_list.append(row_bin)
                        col_bin_list.append(col_bin)
                        magnitude_list.append(weight * gradient_magnitude.item())
                        orientation_bin_list.append((gradient_orientation.item() - angle) * bins_per_degree)
        ...
        threshold = torch.linalg.norm(descriptor_vector) * descriptor_max_value
        descriptor_vector[descriptor_vector > threshold] = threshold
        descriptor_vector /= max((torch.linalg.norm(descriptor_vector).item(), float_tolerance))
        # Complying OpenCV default data type
